 ...
 *
 * - `'user'` (legacy default) maps to `'coordinator'`
 * - Unknown / missing roles fail closed: they must not inherit coordinator
 *   write access. `can()` / `enforce()` deny all actions for the sentinel.
 */
export function normalizeRole(role: string | null | undefined): UserRole {
  if (role === 'admin') return 'admin'
  if (role === 'reviewer') return 'reviewer'
  if (role === 'coordinator' || role === 'user') return 'coordinator'
  // Fail closed — unknown roles match nothing in the permission matrix
  return '__none__' as UserRole
}
