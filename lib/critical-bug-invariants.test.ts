...
describe('readiness ignores non-applicable checklist items', () => {
  it('filters by jurisdictionId and permit type applicability', () => {
    const source = fs.readFileSync(
      path.join(process.cwd(), 'lib/readiness-engine.ts'),
      'utf8'
    )
    assert.match(source, /requirementAppliesToPermitType/)
    assert.match(source, /jurisdictionId !== pkg\.jurisdictionId/)
    assert.match(source, /applicableItems/)
  })
