...
const EXPIRY_CONFIG: Record<ExpiryStatus, { icon: typeof CheckCircle2; label: string; color: string; bg: string }> = {
  ...
  unknown:       { icon: Clock,          label: 'No date set',    color: 'text-muted-foreground/80',   bg: 'bg-muted/40 border-border' },
}
...
export function VaultPanel({ contractorId }: Props) {
  ...
  if (loading) return <div className="p-4 text-sm text-muted-foreground/80">Loading vault…</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {DOC_TYPES.map((type) => {
          ...
          return (
            <div
              key={type}
              className={`border rounded-xl p-4 space-y-2 ${doc ? cfg.bg : 'bg-muted/40 border-border'}`}
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {doc ? (
                    ...
                    <p className="text-xs text-muted-foreground/80 mt-1">Not on file</p>
                  )}
                </div>
                <Icon className={`w-5 h-5 shrink-0 ${doc ? cfg.color : 'text-muted-foreground'}`} />
              </div>
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-1.5 text-xs text-muted-foreground hover:text-foreground border border-input rounded-lg px-3 py-1.5 hover:bg-card transition-colors"
              >
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm border border-input text-muted-foreground hover:bg-muted/40 transition-colors"
              >
                Cancel
              </button>
            </div>
