        ...
        {profiles.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted font-medium">No export profiles yet</p>
              <p className="text-sm text-muted mt-1">Create profiles to control how permit packages are zipped for submission.</p>
              <Link href="/admin/export-profiles/new">
                <Button className="mt-4">Create first profile</Button>
              </Link>
            </CardContent>
          </Card>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {profiles.map(profile => (
              <Card key={profile.id} className="transition-colors hover:bg-surface-inset">
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    <div>
                      <CardTitle className="text-base">{profile.name}</CardTitle>
                      {profile.jurisdiction && (
                        <p className="text-xs text-muted mt-0.5">{profile.jurisdiction.name}</p>
                      )}
                    </div>
                    {profile.isDefault && (
                      <span className="text-xs bg-accent-muted text-accent px-2 py-0.5 font-medium">Default</span>
                    )}
                  </div>
                </CardHeader>
                <CardContent>
                  {profile.description && (
                    <p className="text-sm text-muted mb-3 line-clamp-2">{profile.description}</p>
                  )}
                  <div className="flex items-center justify-between text-xs text-muted">
                    <span>{profile._count.exportLogs} export{profile._count.exportLogs !== 1 ? 's' : ''}</span>
                    <Link href={`/admin/export-profiles/${profile.id}`} className="text-accent hover:underline">
                      Edit
                    </Link>
                  </div>
                </CardContent>
