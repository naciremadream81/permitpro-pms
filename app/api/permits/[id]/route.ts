...
export async function PATCH(
  ...
  try {
    ...
    // Detect a jurisdiction (re)assignment so we can regenerate the checklist.
    const jurisdictionChanged =
      validatedData.jurisdictionId !== undefined &&
      validatedData.jurisdictionId !== currentPermit.jurisdictionId

    // Mirror the human-readable county name into the legacy free-text field so
    // existing displays/reports stay populated when a jurisdiction is assigned.
    let newJurisdiction: { id: string; name: string } | null = null
    if (jurisdictionChanged && validatedData.jurisdictionId) {
      newJurisdiction = await prisma.jurisdiction.findUnique({
      ...
      data.county = newJurisdiction.name
    }
    ...
    // Regenerate the checklist from the new jurisdiction's requirements.
    ...
    if (jurisdictionChanged) {
      ...
      await prisma.activityLog.create({
        data: {
          ...
          description: newJurisdiction
            ? `Jurisdiction set to ${newJurisdiction.name} — checklist synced (${result.generated} added)`
            : 'Jurisdiction cleared — checklist synced',
          oldValue: currentPermit.jurisdictionId,
          newValue: validatedData.jurisdictionId ?? null,
        },
      })
    }
